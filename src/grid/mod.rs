...
impl Grid {
    ...
    fn get_index(&self, row: i32, col: i32) -> Option<usize> {
        ...
        } else if row >= 0 && row < self.num_rows as i32 && col >= 0 && col < self.num_cols as i32 {
    ...
    pub fn get_cell(&self, row: i32, col: i32) -> Option<&Cell> {
        self.get_index(row, col).and_then(|index| self.lattice.get(index))
    }
    
    #[inline]
    pub fn get_cell_mut(&mut self, row: i32, col: i32) -> Option<&mut Cell> {
        self.get_index(row, col).and_then(move |index| self.lattice.get_mut(index))
    }
}
...
mod tests {
    ...
    fn test_preset_grid() {
        ...
        assert_eq!(grid.lattice, expected, "Grid should be lone defector based if RngSettings is None");
    }
